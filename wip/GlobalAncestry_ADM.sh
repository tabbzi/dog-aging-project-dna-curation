...
paste - ${input}'_GlobalAncestrySNPs_RefMerge.'${K}'.Q' \
  > ${input}'_GlobalAncestrySNPs_RefMerge.'${K}'.Q.labeled'

# parse outputs
python3 parseGlobalAncestryADM.py \
  -P /home/tcomi/projects/dog-aging-project-dna-curation/ref/REDCap_Population_Key.tsv \
  -S <(cut -f 2 ${input}'_biallelic-snps.fam') \
  -Q ${input}'_GlobalAncestrySNPs_RefMerge.'${K}'.Q.labeled' \
  -O testing/parseADM/
